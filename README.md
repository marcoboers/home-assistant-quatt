...
2. **Optional: `OduType` sensor enabled**: The `OduType` (ODU Type) sensor is used to select the correct heat pump image.
   ...
   - Find the `OduType` sensor, you will find it under the Diagnostics of the Heatpump (if you have two heatpumps enable them both)
...
- **Card not found**:
  - Ensure Home Assistant has loaded the integration properly. Does the config have a `quatt-dashboard-card.js` file in `<HA config>/custom_components/quatt/www/js/`?
