"""Binary sensor platform for quatt local API."""
...
async def async_setup_entry(hass: HomeAssistant, entry, async_add_devices):
    """Set up the local binary_sensor platform."""
    ...
    _LOGGER.debug("Local - Heatpump 1 active: %s", heatpump_1_active)
    _LOGGER.debug("Local - Heatpump 2 active: %s", heatpump_2_active)
    _LOGGER.debug("Local - All electric active: %s", all_electric_active)
    _LOGGER.debug("Local - boiler OpenTherm: %s", is_boiler_opentherm)
    ...
    sensors: list[QuattLocalBinarySensor] = []
    for device_id, sensor_descriptions in BINARY_SENSORS.items():
        sensors.extend(
            QuattLocalBinarySensor(
...
class QuattLocalBinarySensor(QuattEntity, BinarySensorEntity):
    """quatt local binary_sensor class."""
